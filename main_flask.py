...
cap = None
ARDUINO_PORT = "COM12"
CLASS_CONFIDENCE = 0.7  # Change this value if you want
...
def clean_class_name(class_name):
    # Extracts the material type (plastic, metal, glass, etc.) from the label
    if class_name and isinstance(class_name, str):
        match = re.search(r'^\d+\s+(\w+)$', class_name.strip())
        if match:
            return match.group(1).lower()  # Return the name in lowercase
        else:
...
def initialize_system():
    ...
    try:
        ...
        if not cap.isOpened():
            print("⚠️ Error: Cannot open camera.")
            return False
    ...
    try:
        if arduino is not None and arduino.is_open:
            arduino.close()
            print("🔄 Serial port already open, closing before reconnecting.")
        ...
        # Send RESET command to stepper motor
        arduino.write("RESET\n".encode())
...
# Bin commands mapped to class names
bin_commands = {
    "plastic": "CW",            # Bin 1
    "metal": "CW CW",           # Bin 2
...
# Get the corresponding motor command
